            ...
            <div class="Searchdetails">
                <form action="{{ url('/filter/tour') }}" method="GET" id="filter" name="myform" >
                    <div class="row searchproduct">
                        ...
                        <div class="col-md-12">
                            <div class="input-group searchbox">
                                <input type="text" name="searchByname" id="searchByname" value="@if(isset($_GET['searchByname']) ){{ $_GET['searchByname'] }}@endif" class="form-control" placeholder="Search Tours..">  
                    ...
                    <div class="row detailsproduct">
                        <div class="col-md-4">
                            <input type="search" name="searchByLoc" id="searchBy" value="@if(isset($_GET['searchByLoc']) ){{ $_GET['searchByLoc'] }}@endif" class="form-control" placeholder="Enter Location">
                        </div>
                                ...
                                @if ( isset($arrOwner) && count($arrOwner) > 0 )
                                    ...
                                    @foreach ($arrOwner as $value)
                                        @if(isset($_GET['owner_id']) && $_GET['owner_id'] == $value->user_id )
                                            <option value="{{ $value->user_id }}" selected>{{ ucfirst($value->GetOwner->name) }}</option>
                                        @else
                                            <option value="{{ $value->user_id }}">{{ ucfirst($value->GetOwner->name) }}</option>
                                        @endif
                                    @endforeach
                                @endif
                            </select>
                    ...
                    <div class="buttonblock row">
                        <div class="col-md-12 ">
                            <input type="submit" class="btn btn-default searchbtn"  value="Search" style="background-color: #2196f3;color: #fff;">
                            <a href="{{ url('tour/view') }}"><input type="button" class="btn btn-default searchbtn" value="Reset" style="background-color: #2196f3;color: #fff;"></a>
                        </div>
                    </div>     
                </form>
            </div>
        ...
        <div class="card-columns">
            ...
            <div class="card">
                <div class="card-header bg-white center">
                    <h4 class="card-title"><a href="{{ url('/tour/show/'.base64_encode($tour['id'])) }}">{!! ucfirst(substr($tour['tour_name'], 0, 40)) !!}</a></h4>
                </div>
                <a href="{{ url('/tour/show/'.base64_encode($tour['id'])) }}">
                    <img src="{!! asset('assets/tourPhotos/'.$tour['get_images']['0']['tour_image']) !!}" class="productimg" alt="Card image cap" style="width:100%;max-height: 285px;">
                </a>
                <div class="card-block">
                    ...
                    <div class="form-group row">
                        <span class="text-success" style="font-size:21px;">${!! number_format($tour['tour_price'],2) !!} {{-- + $1* --}}</span> 
                        <div class="col-sm-8">
                            <div class="row">
                                <div class="col-md-6">
                                </div>
                                <div class="col-md-6">
                                    <a href="{{ url('/book/tour/'.base64_encode($tour['id'])) }}" ><button type="button"  class="btn btn-primary btn-rounded">Book Now</button></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-8">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <span class="text-success" style="font-size:21px;">${!! number_format($tour['tour_price'],2) !!}</span>
