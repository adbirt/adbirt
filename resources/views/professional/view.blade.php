...
@media screen and (min-width: 401px) and (max-width: 1024px) {
          ...
          <div class="card-header bg-white">
            <div class="media">
              <div class="media-left media-middle">
                <a href="">
                {!! Html::image($professional->profile->propic, 'User profile picture', array('class' => 'img-responsive img-square', 'width' => '110px', 'height' => '110px')) !!}
                </a>
              </div>
              <div class="media-body media-middle">
			<h5 style="text-transform:uppercase;">{{ str_limit($professional->name, $limit = 25, $end = '') }}</h5> <a href="#" class="btn btn-success btn-rounded" style="text-transform:capitalize;">{{ str_limit($professional->profile->profession, $limit = 25, $end = '') }} <i class="material-icons">add</i></a>
				<h3 class="card-title"><a href="" style="text-transform:capitalize;">{{ str_limit($professional->profile->city, $limit = 13, $end = '') }}, {{ str_limit($professional->profile->state, $limit = 12, $end = '') }}</a></h3>
                <div>
 ...
 <a href="tel:{!! $professional->phone !!}" class="btn btn-primary btn-sm pull-xs-left call"><i class="material-icons">call</i> Call Now</a>
 <a href="{!! URL::route('getcontact') !!}" class="btn btn-default btn-sm pull-xs-right"><i class="material-icons">mail</i> Report </a>
              </div>
            </div>
          </div>
        </div>	
